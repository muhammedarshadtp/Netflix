    ...
    return (
        <div className="pt-[40%] md:pt-[12%] flex justify-center">
            <form className="bg-black w-full md:w-1/2 grid grid-cols-12 rounded-md" onSubmit={(e)=>e.preventDefault()}>
                <input className="p-1 md:p-4 m-4 col-span-9 outline-none rounded-md" ref={searcText} type="text" placeholder={lang[langKey].gptPlaceholder} />
                <button className="col-span-3 bg-red-600 m-4 py-2 px-2 text-white rounded-lg font-medium " onClick={handdleGptSearch} >{lang[langKey].search}</button>
            </form>
        </div>
    )
}
